...
void TextSelect::handleMouseDown(const ImVec2& cursorPosStart) {
    ...
    if (enableWordWrap) {
        ...
        // Calculate wrapped line sizes.
        ImVector<ImVec2> lineSizes;
        lineSizes.reserve(numLines);
        for (std::size_t i = 0; i < numLines; ++i) {
            std::string_view line = getLineAtIdx(i);

            ImVec2 lineSize = ImGui::CalcTextSize(line.data(), line.data() + line.size(), false, wrapWidth);
            lineSizes.push_back(lineSize);
        }

        // Find the index of the line under the cursor.
        float accumulatedHeight = 0;
        y = numLines - 1;
        for (std::size_t i = 0; i < numLines - 1; ++i) {
            if (mousePos.y < accumulatedHeight + lineSizes[i].y) {
                y = i;
                break;
            }
            accumulatedHeight += lineSizes[i].y + ImGui::GetCurrentContext()->Style.ItemSpacing.y;
        }
        if (y >= numLines)
            return;
        ...
        std::size_t localWrappedY = static_cast<std::size_t>(std::min(std::max(std::floor((mousePos.y - accumulatedHeight) / textHeight), 0.0f), std::round(lineSizes[y].y / textHeight) - 1));
